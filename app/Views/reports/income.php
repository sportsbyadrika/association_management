...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
        ...
        <h1 class="mt-1 text-2xl font-bold text-gray-900">Income Report</h1>
    </div>
    <div class="flex gap-2">
        <a href="<?= e(url('/reports/income?' . $qs . '&format=csv')) ?>" class="btn-secondary btn-sm">CSV</a>
        <a href="<?= e(url('/reports/income?' . $qs . '&format=pdf')) ?>" class="btn-primary btn-sm">PDF</a>
    </div>
</div>

<form method="get" action="<?= e(url('/reports/income')) ?>" class="card card-body mb-6 flex flex-wrap items-end gap-3">
    <div>
    ...
    <?php if ($from || $to): ?><a href="<?= e(url('/reports/income')) ?>" class="btn-secondary">Reset</a><?php endif; ?>
    <div class="ml-auto text-right">
        <p class="text-sm text-gray-500">Total income</p>
        <p class="text-2xl font-bold text-brand-700">₹ <?= money($report['total']) ?></p>
    </div>
</form>

<div class="grid gap-6 lg:grid-cols-2">
    <div class="card overflow-hidden">
        <div class="border-b border-gray-100 px-6 py-4"><h2 class="font-semibold text-gray-900">By income head</h2></div>
