...
final class PdfReport
{
    ...
    public function buildHtml(string $title, array $columns, array $rows, array $meta = [], array $summary = [], array $sections = [], string $mainHeading = ''): string
    {
        ...
        $mainTable = $this->renderDataTable($columns, $rows);
        ...
        foreach ($sections as $section) {
            $sectionsHtml .= '<div class="section-title">' . $esc($section['title'] ?? '') . '</div>'
                . $this->renderDataTable($section['columns'] ?? [], $section['rows'] ?? []);
        }
        ...
        <html><head><meta charset="utf-8"><style>
            * { font-family: 'DejaVu Sans', sans-serif; }
            ...
            table.data td.st-paid { background: #dcfce7; color: #166534; font-weight: bold; }
            table.data td.st-partial { background: #ffedd5; color: #9a3412; font-weight: bold; }
            table.data td.st-pending { background: #fee2e2; color: #991b1b; font-weight: bold; }
    ...
    /**
     * Render a report table with zebra rows, right-aligned money columns and
     * coloured status cells (paid/completed green, partial orange, pending red).
     * @param list<string> $columns
     * @param list<array<int,mixed>> $rows
     */
    private function renderDataTable(array $columns, array $rows): string
    {
        $esc = static fn ($v) => htmlspecialchars((string) $v, ENT_QUOTES, 'UTF-8');
        $moneyKeywords = ['amount', 'received', 'balance', 'total', 'value', 'target', 'collected', 'spent', 'paid', 'debit', 'credit', 'contribution', 'net', 'income', 'expense', 'due', 'outstanding', 'pending', 'collection'];

        // Right-align columns whose header names a money value.
        $numCol = [];
        foreach ($columns as $ci => $c) {
            $h = strtolower((string) $c);
            $num = false;
        ...
        $thead = '';
        foreach ($columns as $ci => $c) {
            $thead .= '<th' . (!empty($numCol[$ci]) ? ' class="num"' : '') . '>' . $esc($c) . '</th>';
        }

        $tbody = '';
        foreach ($rows as $i => $row) {
            $tbody .= '<tr' . ($i % 2 === 1 ? ' class="alt"' : '') . '>';
            $ci = 0;
            foreach ($row as $cell) {
                $classes = [];
                if (!empty($numCol[$ci])) {
                ...
                if (in_array($lc, ['paid', 'completed'], true)) {
                    $classes[] = 'st-paid';
                } elseif ($lc === 'partial') {
                    $classes[] = 'st-partial';
                } elseif ($lc === 'pending') {
        ...
        if ($rows === []) {
            $tbody = '<tr><td colspan="' . max(1, count($columns)) . '" style="text-align:center;color:#9ca3af">No records found.</td></tr>';
        }

        return '<table class="data"><thead><tr>' . $thead . '</tr></thead><tbody>' . $tbody . '</tbody></table>';
    }
